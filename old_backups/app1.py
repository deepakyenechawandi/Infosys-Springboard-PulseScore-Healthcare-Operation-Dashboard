...
def show_methodology_dialog():
    ...
    with c1:
        ...
        st.markdown("<p style='font-size:0.9rem; color:#94a3b8;'>Determines the total hospital beds needed per international safety norms (3 beds/1k ppl).</p>", unsafe_allow_html=True)
    ...
    st.markdown("#### Variable Definitions & Component Logic")
...
with st.sidebar:
    ...
    with st.expander("Intelligence Filters", expanded=False):
        ...
        if st.button("View Methodology & Framework", use_container_width=True, type="secondary"):
...
# Theme Variables
if dark_mode:
    primary_bg, card_bg, card_border, main_text, sub_text, card_shadow, grad_opacity = "#0f172a", "#1e293b", "#334155", "#f8fafc", "#94a3b8", "rgba(0,0,0,0.3)", "0.2"
else:
    primary_bg, card_bg, card_border, main_text, sub_text, card_shadow, grad_opacity = "#f8fafc", "white", "#f1f5f9", "#1e1b4b", "#64748b", "rgba(30,27,75,0.04)", "0.12"
...
<style>
    ...
    [data-testid="stSidebar"] div[role="radiogroup"] label div[data-testid="stRadioButton"] {{ display: none !important; }}
    ...
    [data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked) {{ background: linear-gradient(90deg, rgba(255, 255, 255, 0.15) 0%, rgba(255, 255, 255, 0.05) 100%) !important; border: 1px solid rgba(255, 255, 255, 0.3) !important; font-weight: 800 !important; letter-spacing: -0.2px !important; box-shadow: 0 8px 30px rgba(0, 0, 0, 0.3) !important; }}
